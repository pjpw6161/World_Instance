...
```txt
Portfolio Hub             http://localhost:5173/portfolio
Algorithm Comparison Lab  http://localhost:5173/compare
Determinism Lab           http://localhost:5173/determinism
Editor                    http://localhost:5173/editor
Gallery / Explore         http://localhost:5173/gallery
My Worlds                 http://localhost:5173/dashboard
API Health                http://localhost:8080/api/health
```

> 이 README는 포트폴리오 제출용으로 구성되어 있습니다. 더 긴 3페이지 포트폴리오 작성 자료는 [`docs/24_PORTFOLIO_CASE_STUDY.md`](docs/24_PORTFOLIO_CASE_STUDY.md)에 정리되어 있습니다.
...
```mermaid
flowchart LR
  User[Browser User] --> Web[React / Vite UI]
  Web --> Wasm[C++ / WebAssembly Engine]
  Wasm --> MapData[MapData]
  MapData --> Render2D[2D / Height / Side Views]
  MapData --> Render3D[3D Terrain Preview]
  MapData --> ClientSim[Client-side World Instance]
  Web --> Api[Spring Boot API]
  Api --> Postgres[(PostgreSQL Source of Truth)]
  Api --> Elastic[(Elasticsearch Projection)]
```
